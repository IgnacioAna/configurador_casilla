...
| 2 | El plano dibuja las camas como rectángulos marrón con etiqueta C/S/M contra las paredes laterales y el pasillo central. | VERIFIED (2026-07-14) | `configDesdeEstado` pasa `dormitorio.camas` al plano; `FloorPlan.jsx` itera `layout.camas` y renderiza `<Bed>`. Verificado en browser vivo (audit-fix F-02): con config 1C+2S el plano SVG renderiza textos "C","S","S" y rects de cama fill `#B5915A` (marrón) dentro de la zona dormitorio `#C9A66B`; zonas baño `#BFE3EE` y cocina `#A7C796` presentes. Además el UAT E2E de Fase 7 (07-03) aprobó la verificación visual en vivo a ~375px. |
...
**Score:** 5/5 truths VERIFIED. Truth 2 (visual) cerrada 2026-07-14 vía verificación en browser vivo (audit-fix F-02) + UAT E2E aprobado en Fase 7. El FAILED de truth 4 (cajonera) fue resuelto en commit 9d2dc19.
...
| DORM-03 | 05-03, 05-05 | Plano dibuja camas marrón C/S/M + pasillo central | SATISFIED | `configDesdeEstado` pasa `dormitorio.camas`; `FloorPlan.jsx` itera `layout.camas` → `<Bed>`. Verificado en browser vivo 2026-07-14 (audit-fix F-02): camas "C","S","S" con fill #B5915A. |
...
| COCINA-04 | 05-03, 05-05 | Plano dibuja zona cocina/estar verde con módulos elegidos | SATISFIED | `Kitchen` montado condicionalmente, `Table` con `tieneMesa`. Zona cocina verde #A7C796 verificada en browser vivo 2026-07-14 (audit-fix F-02); visual a ~375px aprobado en UAT E2E de Fase 7. |
